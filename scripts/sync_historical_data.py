"""
Sync Historical Data from Oracle Cloud to Local
Automatically downloads new historical data files from Oracle Cloud.
Run by Windows Task Scheduler every 6 hours.
"""
...
# Paths
ORACLE_DATA_DIR = "/home/ubuntu/projects/forex/data_cleaned/dukascopy_local"
LOCAL_DATA_DIR = Path(__file__).parent.parent / "data_cleaned" / "dukascopy_local"

# Setup logging
log_file = Path(__file__).parent.parent / "logs" / "historical_sync.log"
...
def sync_historical_data():
    """Sync all historical data from Oracle Cloud to local."""
    logger.info("=" * 60)
    logger.info("Starting historical data sync from Oracle Cloud")
    logger.info("=" * 60)
    ...
    try:
        ...
        try:
            remote_files = sftp.listdir(ORACLE_DATA_DIR)
        except FileNotFoundError:
            logger.error(f"Oracle Cloud directory not found: {ORACLE_DATA_DIR}")
            return

        logger.info(f"Found {len(remote_files)} files on Oracle Cloud")

        # Get list of local files
        local_files = {f.name for f in LOCAL_DATA_DIR.glob('*.csv')}
        ...
        for filename in remote_files:
            ...
            remote_path = f"{ORACLE_DATA_DIR}/{filename}"
